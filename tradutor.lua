...
-- Tradução intllib
revom.intllib = {}
revom.intllib.S, revom.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
revom.S = revom.intllib.S
revom.NS = revom.intllib.NS
...
-- Gera arquivos de tradução revom.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por revom apartir de "..file.."\n# textdomain: revom\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/revom."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("revom")
else
	s = revom.intllib.S
end
...
revom.s = function(...)
	...
	minetest.log("error", "[revom] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
	...
	if type(args[1]) == "table" then
		...
		for n,a in ipairs(args[1]) do
			if n ~= 1 then -- Não traduz o primeiro
				table.insert(r, revom.S(a))
			else
				table.insert(r, a)
			end
		end
		
		return revom.s(unpack(r))
...
revom.Sfake = function(s) return s end
